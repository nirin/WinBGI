...
BOOL registerWindowClass()
{
	...
	wcex.hIcon = nullptr;                         // Use default icon
	wcex.hIconSm = nullptr;                       // Default small icon
	wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);       // Set mouse cursor
	wcex.hbrBackground = GetStockBrush(BLACK_BRUSH);  // Background color
	wcex.lpszMenuName = nullptr;                  // Menu identification
	wcex.lpszClassName = "BGILibrary";  // a c-string name for the window
...
)
{
	...
	case DLL_PROCESS_ATTACH:
		...
		return registerWindowClass() == FALSE ? false : true;    // Register the window class for this process
		// break;
...
void showerrorbox(const char* msg)
{
	...
	if (msg == nullptr)
	{
		...
		FormatMessage(
			...
			nullptr,                                      // Location of message definition
		...
		MessageBox(nullptr, LPCTSTR(lpMsgBuf), "Error", MB_OK | MB_ICONERROR);
...
void graphdefaults()
{
	...
	pWndData->t_scale[0] = 1; // mul-x
	pWndData->t_scale[1] = 1; // div-x
	pWndData->t_scale[2] = 1; // mul-y
	pWndData->t_scale[3] = 1; // div-y
...
// The initwindow function is typically the first function called by the
...
//                  uniquely identifying the window.
//               On failure, -1.
//
int initwindow(int width, int height, const char* title, int left, int top, bool dbflag, bool closeflag)
{
	...
	auto pWndData = new WindowData;
	// Check if new failed
	if (pWndData == nullptr)
		return -1;

	// TODO: check to make sure the events are created successfully
	pWndData->key_waiting = CreateEvent(nullptr, FALSE, FALSE, nullptr);
	pWndData->WindowCreated = CreateEvent(nullptr, FALSE, FALSE, nullptr);
	...
	hThread = CreateThread(nullptr,  // Security Attributes (use default)
...
void closegraph(int wid)
{
	...
	{
		for (auto i = 0; i < BGI__WindowCount; i++)
		{
			closegraph(i);
		}
	}
	else if (wid >= 0 && wid <= BGI__WindowCount && BGI__WindowTable[wid] != nullptr)
	{
...
// This function detects the graphics driver and returns the highest resolution
...
char *grapherrormsg(int errorcode)
{
	static char *msg[16]
	{
		"No error", "Graphics not installed",
		...
		"Font file not found", "Not enough memory to load font",
	...
	return errorcode < -16 || errorcode > 0 ? nullptr : msg[-errorcode];
}
...
{
	if (window < 0 || window >= BGI__WindowCount || BGI__WindowTable[window] == nullptr)
	{
		return;
	}
...
void setvisualpage(int page)
{
	...
	InvalidateRect(pWndData->hWnd, nullptr, FALSE);
}
