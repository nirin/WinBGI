...
{
	auto alignment = pWndData->alignment;
...
void outtext(char *textstring)
{
	...
	TextOut(hDC, 0, 0, LPCTSTR(textstring), int(strlen(textstring)));
...
void outtextxy(int x, int y, char *textstring)
{
	...
	TextOut(hDC, x, y, LPCTSTR(textstring), int(strlen(textstring)));
	BGI__ReleaseWinbgiDC();
...
int textheight(char *textstring)
{
	...
	GetTextExtentPoint32(hDC, LPCTSTR(textstring), int(strlen(textstring)), &tb);
	BGI__ReleaseWinbgiDC();
...
void outstreamxy(int x, int y, ostringstream& out)
{
	string all, line;
	auto startx = x;
	...
	for (size_t i = 0; i < all.length(); i++)
	{
		...
		{
			if (line.length() > 0)
				outtext(const_cast<char *>(line.c_str()));
			y += textheight("X");
			x = startx;
			line.clear();
			moveto(x, y);
